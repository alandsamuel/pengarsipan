...
if ($conn->connect_error) {
  ...
  $sql="select `id_sk`,`id_pegawai`,`tanggalkeluar_sk`,`sifat_sk`,`no_sk`,`tanggalsurat_sk`,`asal_sk`,`perihal_sk`,`lampiran_sk`,`gambar`,`catatan_sk`,`bagian_sk`,`status_sk`,`status_bagian`,`lokasi_sk` from `$tbsuratkeluar` order by `id_sk` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
