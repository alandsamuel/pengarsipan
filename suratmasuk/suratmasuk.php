...
if($_GET["pro"]=="ubah"){
	...
	$d=getField($conn,$sql);
                ...
                $klasifikasi = $d['klasifikasi_surat'];
                $no = $d['no_surat'];
                $tgl = $d['tanggal_surat'];
                $sifat = $d['sifat_surat'];
...
<tr>
    <td style="width:100px;border: 0px">Asal Surat</td>
    <td style="width:400px;border: 0px">&nbsp; 
        <select name="asal_surat"><option>Asal Surat</option>
        <?php
    
        foreach($dir as $r => $r_val){
...
<tr>
    ...
    <td style="width:400px;border: 0px"> &nbsp;
    <input type="date" name="deadline" value="<?php echo $deadline;?>"><p style="color:red;margin-bottom:0;padding:0;">* FORMAT = YYYY-MM-DD *</p></td>
</tr>
<tr></tr>
<tr>
...
<input type="text" value="sm" name="tipe_surat" HIDDEN><br>
    </center>
    </form>
...
<?php  
    $tipe = 'sm';
  $sql="select * from `$tbsurat` where tipe_surat = '$tipe' order by id_surat DESC";
  $jum=getJum($conn,$sql);
		if($jum > 0){
	//--------------------------------------------------------------------------------------------
	$batas   = 2;
...
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $id_sm pada data suratmasuk ?..\")'></a></td>
				</tr>";
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=suratmasuk'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=suratmasuk'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=suratmasuk'>Next »</a></span>";
	}
...
if ($conn->query($sql) === TRUE) {
    echo "<script>alert('Data $id_surat berhasil disimpan !');document.location.href='?mnu=suratmasuk';</script>";
} else {
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $id_surat berhasil diubah !');document.location.href='?mnu=suratmasuk';</script>";}
