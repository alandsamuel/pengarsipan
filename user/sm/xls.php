...
if ($conn->connect_error) {
  ...
  $sql="select `id_sm`,`id_pegawai`,`tanggalterima_sm`,`sifat_sm`,`no_sm`,`tanggalsurat_sm`,`asal_sm`,`perihal_sm`,`lampiran_sm`,`gambar`,`catatan_sm`,`bagian_sm`,`status_sm`,`status_bagian`,`lokasi_sm` from `$tbsuratmasuk` where bagian_sm = '$bag' order by `id_sm` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
