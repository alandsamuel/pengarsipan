...
<tr>
    ...
    <td style="width:100px;border: 0px">&nbsp; <input type="text" name="nourut_surat" placeholder="No Urut" value="<?php echo $nourut;?>" DISABLED></td>
</tr>
<tr>
...
<?php  
    $tipe = 'sk';
  $sql="select * from `$tbsurat` where tipe_surat = '$tipe' ";
  $jum=getJum($conn,$sql);
		if($jum > 0){
	//--------------------------------------------------------------------------------------------
	$batas   = 5;
...
echo"</td>
				<td align='center'>
<a href='?mnu=suratkeluar&pro=ubah&kode=$id_sk'><img src='ypathicon/u.png' alt='ubah'></a>
<a href='?mnu=suratkeluar&pro=hapus&kode=$id_sk'><img src='ypathicon/h.png' alt='hapus' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=suratkeluar'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=suratkeluar'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=suratkeluar'>Next »</a></span>";
	}
...
if ($conn->query($sql) === TRUE) {
    echo "<script>alert('Data $id_surat berhasil disimpan !');document.location.href='?mnu=suratkeluar';</script>";
} else {
